...
export default function QRScannerScreen() {
  ...
  const handleBarCodeScanned = ({ type, data }: { type: string; data: string }) => {
    ...
    try {
      ...
      Alert.alert(
        'خطأ في قراءة الرمز', 
        'فشل في قراءة رمز QR. تأكد من أن الرمز واضح ومقروء.', 
        [
          {
            text: 'إعادة المحاولة',
            onPress: () => setScanned(false)
  ...
  const handleBorrowBook = async () => {
    if (!scannedUser || !selectedBook) {
      Alert.alert('خطأ', 'يرجى اختيار كتاب');
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        Alert.alert(
          'تم بنجاح',
          `تم تسجيل استعارة الكتاب للمستخدم ${scannedUser.name}`,
          [
            {
              text: 'موافق',
              onPress: () => {
